...
let notifications = [];
// let x = path.resolve(__dirname, 'refresh.js')
// console.log(x);
let scripts = [{ script1: "/js/refresh.js" }];
...
router.get('/running', function(req, res){
	console.log("Witness Protection Engaged ---- ".magenta.bold+"@".blue.bold + process.env.witnessUsername.blue.bold);
	let switches = 0;
...
*/
	 ...
	 witness_name = process.env.witnessUsername;
	 receiver = process.env.receiving_email;
	 active_key = process.env.account_ActiveKey;
	 backup_key = process.env.NodeA_BrainKey;
	 mainBrain_key = process.env.NodeB_BrainKey;
	 witness_link = process.env.WitnessLink;
	 account_creation_fee = process.env.accountCreationFee;
	 maximum_block_size = parseInt(process.env.MaximumBlockSize);
	 check_rate = parseInt(process.env.checkRate);
	 email = process.env.NotificationGmail_Email;
	 password = process.env.NotificationGmail_Login;
	 props = {
		account_creation_fee,
		maximum_block_size
	};
	 node_chain_id = process.env.node_chain_id;
	 node_address_prefix = process.env.node_address_prefix;
	 node_URL = process.env.node_URL;
	console.log("Getting Address Prefix from ENV: ".gray.bold ,node_address_prefix.magenta.bold);
	...
	setNodeData(node_address_prefix, witness_name, active_key, backup_key, mainBrain_key, witness_link, account_creation_fee, maximum_block_size, check_rate, email, password, props, receiver, switches);
...
router.get('/update-env', function(req, res){
		res.render('credentials', { scripts: scripts});
...
router.post('/updateENV', function(req, res){
	...
	*/
	...
	let account_creation_fee = req.body.accountCreationFee;
	let maximum_block_size = parseInt(req.body.MaximumBlockSize);
	let  threshold= " ";
	...
	let node_address_prefix = x['Adress_Prefix:'];
	...
	writeENV(witnessUsername, receiving_email,account_ActiveKey, NodeA_BrainKey, NodeB_BrainKey, WitnessLink,
		 account_creation_fee, threshold, checkRate, NotificationGmail_Login, NotificationGmail_Email, maximum_block_size, node_chain_id, node_address_prefix, node_URL);
		 res.redirect('/');
});
